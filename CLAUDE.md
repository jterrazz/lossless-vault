...
- **Flat CLI structure**: All commands are top-level (`add`, `rm`, `scan`, `status`, `dupes`, `pack`, `export`). `pack` = permanent lossless archive (best-quality originals, persists across source removal). `export` = compressed HEIC output for space savings. Both persist their path on first use.
