...
- **Perceptual hash pipeline**: JPEG path: `turbojpeg` 1/4 DCT scaling + GRAY format (skips chroma) → SIMD resize to 9x8. Non-JPEG path: `image` crate → RGB resize to 9x8 → manual BT.601 on 72 pixels. Both produce a 9x8 grayscale buffer for manual aHash + dHash. No `img_hash` dependency. `turbojpeg` is feature-gated (default on, disable with `--no-default-features` for WASM). DCT scaling uses adaptive factor (1/4, 1/2, or full) keeping minimum 32px dimension.
